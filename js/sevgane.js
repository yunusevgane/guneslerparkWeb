...
if(durum>0){
document.getElementById(idart).value = Number(document.getElementById(idart).value)+1;
if (artartislem){
var artartism = artartislem.replace(",", ".");
var res = document.getElementById(artism).innerHTML.replace(",", ".");
document.getElementById(artism).innerHTML = (Number(res)+Number(artartism))+artislem;
}
}else{
...
if (artartislem){
var artartism = artartislem.replace(",", ".");
document.getElementById(artism).innerHTML = Number(document.getElementById(artism).innerHTML)-Number(artartism);
}
}
}	
}; 
...
artv=artva+1
document.getElementById("p").onclick= function(){webyapar.artt(artv,artart,artism,artis)};
artvan=artva-1
document.getElementById("n").onclick= function(){webyapar.artt(artvan,artart,artism,artis)};
...
if(b[ii].innerHTML.toUpperCase().indexOf(x.elements[i].value.toUpperCase()) > -1 && durumm==1 )
{
	b[ii].style.display = "";
...
webyapar.displayObject = function (forlist, data) {
  ...
  for (j = (l - 1); j >= 0; j -= 1) {
	...
	for (x in repeatObj) {
        ...
        i += 1;
        rowClone = arr[j];
        rowClone = matrix_replace_curly(rowClone, "element", repeatObj[x]);
...
if (x && x[lookFor]=="225805" ) {value = x[lookFor]+"["+lookFor+"]";} else { value = undefined;}
		}

else
{
	if (x) {value = x[lookFor];} else { value = undefined;} // degisiklik alaný
}
...
if (pn)
{ 
document.getElementById("p").onclick= function() {webyapar('1')};
document.getElementById("n").onclick= function() {webyapar('-1')};
if (y1){art		= y1;}
else
{art			= 0;}
durum = Number(yapim.value)+Number(art); 
if (Number(durum)>0){art=art;}
else{art=0;}
...
if (bilgim == "img"){
yapim.src = this.innerHTML = "/uye/" + listeme["id"] + "/" + listeme["img"];}
else{
